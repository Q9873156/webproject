...
if (isset($GoUpload) && $GoUpload=='1') {
    ...
    if (!empty($fname) && addslashes($fname)==$fname && $fsize>0) {
        $uploadfile = "$AttachDir/" . str_pad($cid,8,'0',STR_PAD_LEFT) . '.doc';
...
<tr>
<th>
<input type="button" name="upload" value="上傳簡介" onclick="startload()">&nbsp;&nbsp;
...
if (file_exists($filename)) {
    $Tag = date('His');
?>
<div style="text-align:center;margin-top:5px;">原存簡介
